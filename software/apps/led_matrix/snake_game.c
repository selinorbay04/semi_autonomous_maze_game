...
void snake_game_advance_state(void) {
  ...
  // TODO
  switch(game_state.button_pressed){
    case 0:
      break;
    case 14:
      ...
      break;
    case 23:
      game_state.dir == UP ? game_state.dir = RIGHT : game_state.dir--;
      break;
  }
  game_state.button_pressed = 0;
  ...
  if (game_state.iterations % 10 != 0) {
    uint8_t old_tail = queue.tail;

    snakeloc_t tail = queue_pop_tail();
    if (tail.row != -1 && tail.col != -1) {
      game_state.model[tail.row][tail.col] = false;
      queue.slots[old_tail].row = -1;
      queue.slots[old_tail].col = -1;
    }
  }
  // TODO
  ...
  switch (game_state.dir) {
    case UP:
      game_state.head.row -= 1;
      break;
    case LEFT:
      game_state.head.col -= 1;
      break;
    case DOWN:
      game_state.head.row += 1;
      break;
  ...
  if (game_state.head.row < 0 || 
      game_state.head.row > 4 || 
      game_state.head.col < 0 || 
      game_state.head.col > 4) {
        game_state.game_over = true;
  }

  if (game_state.model[game_state.head.row][game_state.head.col] == true){
    game_state.game_over = true;
  }
  //
  ...
  queue_push_head(game_state.head);
  for (int i = 0; i < 25; i++){
    if (queue.slots[i].row != -1 && queue.slots[i].col != -1) {
      game_state.model[queue.slots[i].row][queue.slots[i].col] = true;
    }
  }
  

  // display the game model on the LED matrix
  set_state(game_state.model);
